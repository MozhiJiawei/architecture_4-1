...
`summary_label` is a renderer-facing short label for an edge. It should be generated by the view-owning subagent when the full relationship `label` is too long for a readable diagram edge.
